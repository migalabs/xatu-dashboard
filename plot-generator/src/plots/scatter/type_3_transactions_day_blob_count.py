...
from utils import annotations_add, bold, get_epoch_readable_unit, fraction_clamp
...
def day_type_3_transactions_and_blob_count_create(client):
    plotname = 'scatter_day-type-3-transactions-blob-count'
    ...
    x, y = 'day', ['tx_count', 'blob_count']

    print(df['tx_count'].max())
    hovertemplate = (
        f'{bold("Type 3 transactions")}: %{{y:,.0f}}<br>'
        f'{bold("Blob count")}: %{{customdata[1]:,.0f}}<br>'
    ...
    fig = scatter_create_fig(
        ...
        yskips=fraction_clamp(max(df[y[0]].max(), df[y[1]].max()) / 5, 10),
        xskips=FIVE_DAYS_RATE, xtickformat=DEFAULT_DATE_FORMAT,
        color_discrete_sequence=None, customdata=['slot', y[1]],
